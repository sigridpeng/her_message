...
const InventoryUI: React.FC<InventoryUIProps & { onExamine: (id: ItemId) => void }> = ({
  ...
  return (
    <div className="fixed bottom-8 left-1/2 -translate-x-1/2 w-full max-w-2xl bg-white/40 backdrop-blur-xl border border-white/60 p-5 rounded-[2.5rem] z-[40] shadow-[0_20px_50px_rgba(0,0,0,0.1)]">
      <div className="flex items-center space-x-5 px-4 overflow-x-auto scrollbar-hide">
        <div className="text-orange-900/40 font-black italic mr-4 text-[10px] whitespace-nowrap tracking-[0.3em] uppercase shrink-0">Memories</div>
        ...
        <div className="flex items-center space-x-3">
          ...
          {/* Minimum items display for UI balance */}
          {inventory.length < 5 && Array.from({ length: 5 - inventory.length }).map((_, i) => (
            <div key={`empty-${i}`} className="w-14 h-14 rounded-2xl border-2 border-slate-50 bg-white/20 flex items-center justify-center shrink-0">
